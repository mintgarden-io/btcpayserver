...
namespace BTCPayServer.Services.Altcoins.Chia
{
    public static class ChiaLikeExtensions
    {
        ...
        {
            serviceCollection.AddSingleton(provider => provider.ConfigureChiaLikeConfiguration());
            ...
            serviceCollection.AddSingleton<IPaymentMethodHandler>(
                provider => provider.GetService<ChiaLikePaymentMethodHandler>()
            );
            serviceCollection.AddSingleton<IUIExtension>(
                new UIExtension("Chia/ChiaCheckout", "checkout-end")
            );
            serviceCollection.AddSingleton<IUIExtension>(
                new UIExtension("Chia/ChiaCheckout", "checkout-v2-end")
            );
            serviceCollection.AddSingleton<IUIExtension>(
                new UIExtension("Chia/StoreNavChiaExtension", "store-nav")
            );
            serviceCollection.AddSingleton<IUIExtension>(
                new UIExtension("Chia/CheckoutChiaPostContent", "checkout-v2-bitcoin-post-content")
            );
            serviceCollection.AddSingleton<IUIExtension>(
                new UIExtension("Chia/StoreWalletsNavChiaExtension", "store-wallets-nav")
            );
        ...
        private static ChiaLikeConfiguration ConfigureChiaLikeConfiguration(
        ...
        )
        {
            ...
            {
                var chiaRoot = configuration.GetOrDefault<Uri>(
                    $"{ChiaLikeSpecificBtcPayNetwork.CryptoCode}_root",
                    null
                );
                if (chiaRoot == null)
                {
                    throw new ConfigException(
                        $"{ChiaLikeSpecificBtcPayNetwork.CryptoCode} is misconfigured"
                    );
                }

                var config = Config.Open(
                    Path.Combine(chiaRoot.AbsolutePath, "config", "config.yaml")
                );

                result.ChiaLikeConfigurationItems.Add(
                    ChiaLikeSpecificBtcPayNetwork.CryptoCode,
